...
export default function App() {
  ...
  useEffect(() => {
    configureDriverNotifications().then((token) => { if (token) console.log('Register this driver push token with the backend:', token.data); }).catch((error) => console.warn('Driver notifications could not be configured.', error));
    const receivedSubscription = Notifications.addNotificationReceivedListener(() => setScreen('deliveries'));
    const responseSubscription = Notifications.addNotificationResponseReceivedListener((response) => { const notificationId = response.notification.request.identifier; if (response.actionIdentifier === REJECT_ORDER_ACTION) dismissDriverOrderNotification(notificationId).catch(() => undefined); else if (response.actionIdentifier === ACCEPT_ORDER_ACTION || response.actionIdentifier === Notifications.DEFAULT_ACTION_IDENTIFIER) setScreen('deliveries'); });
  ...
  const selectCalendarDay = (day: number) => { setSelectedDay(day); setMeetingDate(day === 22 ? 'Today' : day === 23 ? 'Tomorrow' : day === 25 ? 'Aug 25' : `Aug ${day}`); };

  return <SafeAreaView style={styles.safeArea}><StatusBar style="light" />{screen !== 'home' && <Pressable onPress={() => setScreen('home')} style={styles.backButton}><Text style={styles.backArrow}>&lt;</Text><Text style={styles.backText}>Back to Home</Text></Pressable>}<ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
    {screen === 'home' && <><HomePage activeDelivery={{ name: activeJob.type, pickup: activeJob.pickup, dropoff: activeJob.dropoff, timing: 'Pickup in 8 min' }} queuedDelivery={{ name: jobs[0].type, pickup: jobs[0].pickup, dropoff: jobs[0].dropoff, timing: jobs[0].time }} online={online} onToggleOnline={() => setOnline((value) => !value)} onOpenProfile={() => setProfileOpen(true)} onOpenEarnings={() => setEarningsOpen(true)} onOpenWithdraw={() => setWithdrawOpen(true)} onOpenDeliveries={() => setScreen('deliveries')} onOpenSurge={() => setScreen('surge')} /><Pressable onPress={() => setScreen('surge')} style={styles.locationCard}><View><Text style={styles.locationKicker}>CURRENT LOCATION</Text><Text style={styles.locationTitle}>Downtown East</Text><Text style={styles.locationText}>Nearest surge: Riverside Market  ·  0.8 mi</Text></View><Text style={styles.locationArrow}>&gt;</Text></Pressable></>}
    ...
    {screen === 'meetings' && <MeetingsPage meetingDate={meetingDate} selectedDay={selectedDay} joinedMeetings={joinedMeetings} dates={dates} days={calendarDays} meetings={meetings} onSelectDate={selectDate} onSelectDay={selectCalendarDay} onToggleJoin={(title) => setJoinedMeetings((items) => items.includes(title) ? items.filter((item) => item !== title) : [...items, title])} />}
